...
df_annual = pd.DataFrame(data={"mean":[round(x,1) for x in data['values'].resample('Y').mean().values],
                               "max":data['values'].resample('Y').max().values,
                               "min":data['values'].resample('Y').min().values,
                               'median':data['values'].resample('Y').median().values},
...
ax.text(0.1,
        0.9,
        f'Slope (a) = {round(m,2)}',    transform=ax.transAxes,
        fontsize=12)
